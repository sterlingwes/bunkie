...
const getWall = (id: string) => bunkieDefinition.components.find(c => c.id === id);
...
const FLOOR_TOP_Y = floorAssembly.position.y + floorAssembly.dimensions.height / 2;
...
describe('Wall Heights Specification', () => {
  ...
  describe('Rake walls meet front/back walls', () => {
    it('south wall front edge should meet front wall top', () => {
      const frontWallTop = frontWall.position.y + frontWall.dimensions.height / 2;
