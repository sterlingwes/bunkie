...
export function WoodStove({ component, showClearance = true }: WoodStoveProps) {
  ...
  const { selectedComponentId, hoveredComponentId, hoverComponent, selectComponent, showClearances } = useBunkieStore();
  ...
  return (
    ...
    >
      ...
      <mesh position={[0, -component.dimensions.height / 2 + 0.01, 0]} receiveShadow>
      ...
      <mesh
        ...
        onClick={handleClick}
      >
        <boxGeometry args={[component.dimensions.width, component.dimensions.height, component.dimensions.depth]} />
        <meshStandardMaterial color={stoveColor} roughness={0.3} metalness={0.8} />
      </mesh>

      {/* Stove legs */}
      {[
        [-component.dimensions.width / 2 + 0.05, -component.dimensions.height / 2 + 0.03, -component.dimensions.depth / 2 + 0.05],
        [component.dimensions.width / 2 - 0.05, -component.dimensions.height / 2 + 0.03, -component.dimensions.depth / 2 + 0.05],
        [-component.dimensions.width / 2 + 0.05, -component.dimensions.height / 2 + 0.03, component.dimensions.depth / 2 - 0.05],
        [component.dimensions.width / 2 - 0.05, -component.dimensions.height / 2 + 0.03, component.dimensions.depth / 2 - 0.05],
      ].map((pos, i) => (
      ...
      <mesh position={[0, 0.05, component.dimensions.depth / 2 + 0.001]}>
        <boxGeometry args={[component.dimensions.width * 0.6, component.dimensions.height * 0.5, 0.005]} />
        <meshStandardMaterial color="#1a1a2e" transparent opacity={0.6} roughness={0.1} />
      </mesh>
